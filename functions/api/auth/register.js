...
async function signToken(payload, env) {
  return new SignJWT(payload).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("8h").sign(getSecret(env));
}
...
async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (password.length < 6) return json({ error: "Contrase\xF1a m\xEDnimo 6 caracteres" }, 400);
    const sql = getDb(env);
    const emailLow = email.toLowerCase();
    const exists = await sql`SELECT id FROM usuarios WHERE email=${emailLow}`;
    if (exists.length) return json({ error: "El email ya est\xE1 registrado" }, 400);
