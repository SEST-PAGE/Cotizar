...
// src/api/quotes/index.js
function numCot() {
  const d = /* @__PURE__ */ new Date();
  return `COT-${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, "0")}-${Math.floor(Math.random() * 9e3) + 1e3}`;
}
async function onRequest({ request, env }) {
  ...
  try {
    if (request.method === "GET") {
      ...
      if (id) {
        ...
        if (own.length) {
          ...
          return json({ ...own[0], items: items2 });
        }
    ...
    if (request.method === "POST") {
      ...
      if (!titulo) return json({ error: "El t\xEDtulo es requerido" }, 400);
    ...
    if (request.method === "PUT") {
      ...
      if (!own.length) {
        ...
        if (!shared.length) return json({ error: "No tienes permiso para editar esta cotizaci\xF3n" }, 403);
      }
      ...
      if (_fullEdit && items) {
        ...
        const total = base + iv;
        const cliId = cliente_id ? parseInt(cliente_id) : null;
        const r2 = await sql`UPDATE cotizaciones SET
